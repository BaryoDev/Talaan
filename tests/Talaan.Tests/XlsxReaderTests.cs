...
public class XlsxReaderTests
{
    ...
    public void Reads_real_roster_when_available()
    {
        ...
        // Find REDACTED REDACTED and verify the surrounding cells parse correctly.
        int dindoRow = -1;
        for (var r = headerRow + 1; r < sheet.RowCount; r++)
        {
            if (sheet.At(r, firstNameCol).AsString().Trim() == "REDACTED" &&
                sheet.At(r, lastNameCol).AsString().Trim() == "REDACTED")
            {
                dindoRow = r;
                break;
            }
        }
        Assert.True(dindoRow >= 0, "expected member 'REDACTED REDACTED' not found");
        ...
        Assert.Equal("REDACTED", memberNo.AsString());
